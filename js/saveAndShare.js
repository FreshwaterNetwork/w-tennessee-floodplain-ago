...
function createURL(){
	...
	var linkRequest = {
		destination: "https://maps.freshwaternetwork.org/dev/western-tennessee-floodplain-webmap/?search=" + uri
	};
	shortUrl = "";
	$.ajax({
...
function buildFromState(){
	if (app.obj.stateSet == "yes"){
		...
		//zoom to layer
		// wait for webmap to load
		app.webmap.when(function(){
		    // loop through layers
		    app.webmap.layers.find(function(layer) {
		        // find selected layer
		        if (layer.title == app.obj.hucLayer){
		            // wait for layer to load
		            layer.watch("loaded", () => {
						// zoom to layer
						app.view.goTo(layer.fullExtent).catch(function(error){
