function mapClick(){
	...
	function( QueryTask, Query) {
		app.view.on("click",function(event){
		  	...
		  	//get feature layer number
		  	let flayer = "";
			Object.entries(app.topObj["toggleBtns"]).forEach(([key, v]) => {
				Object.entries(v.btns).forEach(([key1, v1]) => {
					if (v1.value == app.obj.hucLayer){
						flayer = v1.layer;
					}
				})
			})
		  	...
		  	var queryTask = new QueryTask({
		    	url: "https://services.arcgis.com/F7DSX1DSNSiWmOqh/ArcGIS/rest/services/Western_Tennessee_Watersheds/FeatureServer/" + flayer 
		  	})
			var query = new Query();
		    	...
		    	.then(function(response){
					...
					if (response.features[0]){ 
						...
						app.resultsLayer.addMany(features);
					   	// loop through layers
						app.webmap.layers.find(function(layer) {
							// find map click layer
							if (layer.title == "mapclick"){
								layer.visible = true;
							}
						})
						...
						if (app.obj.floodFreq == 1){
							acresLabel = "Current population in 5-year floodplain";
					   	...
					   	if (app.obj.floodFreq == 2){
					    	acresLabel = "Current population in 100-year floodplain";
					   	...
					   	if (app.obj.floodFreq == 3){
					    	acresLabel = "Current population in 500-year floodplain";
